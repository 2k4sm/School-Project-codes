import mariadb as sqlctr
import sys
from datetime import datetime
mycon = sqlctr.connect(host='localhost', user='root', password='heaven')
if mycon.connection_id != None:
    print('\n')
...
cursor.execute("create table if not exists borrower(SN int(5),borrowers_name varchar(40),book_lent varchar(20),date DATE,contact_no varchar(15))")
def command(st):
    cursor.execute(st)
def fetch():
...
def lend():
    ...
    while flag=='True':
        ...
        contact=str(input('Enter contact no. : '))
...
def tfine():
    name=str(input('\nEnter borrower name : '))
    contact=str(input('Enter borrower contact_no : '))       
    detail_burrower(name, contact)
